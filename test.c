...
void test3(void) {
	...
	printf("Try to create a process with an illegal priority \n");
 ...
 **************************************************************************/
#define         NUMBER_OF_TESTX_ITERATIONS     15
...
void test21(void) {
	...
	char read[PGSIZE];
	Z502ReadPhysicalMemory(0, (char *) read);
	printf("input is %d\n", DataWritten);
	printf("this is %d \n", (int) read);
...
void testS(void) {
	...
	printf("%d\n", MEM_ADJUST(proc_info[ld->OurSharedID].structure_tag));
	...
	if (ld->OurSharedID != 0) {  //   We are a SLAVE Process
		// The slaves keep going forever until the master tells them to quiti
		// int p = 20;
		while (TRUE) {
			...
			if (ReadWriteData != ld->MessageSenderPid) {
				...
				printf("sender correct ID = %d, Mail Box actualId = %d \n",ld->MessageSenderPid, ReadWriteData);
			}
